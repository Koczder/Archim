...
@Composable
fun DirectoryContentGridScreen(
    ...
    val gridState = remember(currentDirUri) {
        LazyGridState(firstVisibleItemIndex = saved.index, firstVisibleItemScrollOffset = saved.offset)
    }

    val clipboardManager = LocalClipboardManager.current

    BackHandler {
        saveScroll(currentDirUri, gridState.firstVisibleItemIndex, gridState.firstVisibleItemScrollOffset)
        onBack()
    }
    ...
    if (actualPreviewMode == PreviewGenerationMode.DIALOG && viewModel.shouldShowPreviewDialog) {
        PreviewGenerationDialog(
            isVisible = true,
            onDismiss = {
                Log.d("DirectoryContentGridScreen", "Dialog dismissed")
                viewModel.hidePreviewDialog()
                onNewDirectoryPreviewHandled?.invoke()
            },
            onConfirm = {
                Log.d("DirectoryContentGridScreen", "Dialog confirmed")
                viewModel.hidePreviewDialog()
                onNewDirectoryPreviewHandled?.invoke()
